...
export default class PhotosApi {
	...
	async request(
		...
		if (resp.status === 200) {
			...
			// Malformed input
			throw new Error(
				"⚠ Malformed input. Please check the filters you are using."
			);
	 ...
	 */
	async mediaItemsSearch(
		options: object = {}
	): Promise<GooglePhotosMediaItemsSearch> {
	...
	async listAlbums(): Promise<GooglePhotosAlbumSearch> {
		...
		const sharedAlbums = (await this.request(
			"/v1/sharedAlbums?pageSize=50",
			{
				method: "GET",
			}
		)) as unknown as GooglePhotosAlbumSearch;
		if (sharedAlbums?.sharedAlbums) {
			for (const sharedAlbum of sharedAlbums.sharedAlbums) {
				...
				if (
					sharedAlbum.title &&
					!albums.albums.find((album) => album.id === sharedAlbum.id)
				) {
...
export function dateToGoogleDateFilter(date: Date) {
